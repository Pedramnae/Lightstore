...
import useFetch2 from '../store/myfetch2'
...
import banner from './../assets/img/bathroomlightningbanner.jpg'

function Bathroomlightning() {

    const addtobasket = useBasket((state) => state.addTobasket)
    const getdata = useFetch2((state) => state.getData)
    const mydata = useFetch2((state) => state.mydata2)
        ...
        <>
            ...
            <section className='w-full h-[40vh] flex flex-wrap px-[50px] relative'>
                ...
                <div className='w-full h-full flex flex-wrap relative z-[2] content-center'>
                    <h1 className='w-full h-fit py-[20px] font-futuram text-white text-[28px]'>Bathroom Lighting</h1>
                    <p className='w-1/2 h-fit font-futuras py-[10px] text-white'>With the right lighting in the bathroom, you can create a delightful and welcoming environment to enjoy. The bathroom is an important place in the home where we spend a lot of time. Combine different bathroom lights for effective and practical lighting.</p>
                </div>
            </section>
            <section className='w-full h-fit py-[40px] flex flex-wrap justify-center'>
                <div className='w-[95%] md:w-[85%] h-[90%] flex flex-wrap content-between justify-between'>
                    {mydata && mydata.map((item) => {
                        if (item.category == 'Bathroom Lighting') {
                            return (
...
export default Bathroomlightning